...
import ru.itis.anifox.dto.StudioDto;
...
public class AnimeMapstruct implements BaseMapstruct<AnimeDto, AnimeEntity> {
    private final GenreRepository genreRepository;

    private final StudioRepository studioRepository;

    @Override
    public AnimeEntity convertTo(AnimeDto animeDto) {
        ...
        animeEntity.setStudio(studioRepository.findByName(animeDto.getStudio().getName())
    ...
    @Override
    public AnimeDto convertFrom(AnimeEntity animeEntity) {
        AnimeDto animeDto = AnimeDto.builder()
                ...
                .avatar(animeEntity.getAvatar().getStorageFileName())
                .id(animeEntity.getId())
                .rating(animeEntity.getRating())
                .build();
        StudioDto studioDto = StudioDto.builder()
                .name(animeEntity.getStudio().getName())
                .avatar(animeEntity.getStudio().getAvatar().getStorageFileName())
                .build();
                animeDto.setStudio(studioDto);
    ...
    @Override
    public List<AnimeEntity> convertTo(List<AnimeDto> animeDtos) {
        return animeDtos.stream().map(this::convertTo).collect(Collectors.toList());
    }

    @Override
    public List<AnimeDto> convertFrom(List<AnimeEntity> animeEntities) {
